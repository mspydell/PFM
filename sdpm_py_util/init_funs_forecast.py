...
def sanitize_all_restart_files(pkl_fnm, hour_step=6, verbose=True):
    ...
    Meant to be called once at the top of the forecast driver, before any
    per-level restart_setup fires. This guarantees every candidate restart
    file has clean timestamps by the time ROMS reads it, so the per-level
    sanitizer inside get_restart_file_and_index becomes a fast no-op and
    ROMS's bc/dstart consistency check can't be tripped by 1-timestep or
