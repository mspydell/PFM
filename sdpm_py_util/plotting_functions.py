...
def plot_atm_fields(ATM, RMG, product_name, fields_to_plot=None):
    """
    ...
    for field in fields_to_plot:
        ...
        if field == 'velocity':
            ...
            cbar = fig.colorbar(cset, ax=ax, orientation='horizontal')
...
def plot_all_fields_in_one(ATM, ATM_R, RMG, product_name, fields_to_plot=None):
    """
    Plot specified fields from both the ATM and ATM_R datasets with timestamps and product names, and save them in one PNG file.
    ...
    fig, axs = plt.subplots(nrows=num_fields, ncols=2, figsize=(16, 12 * num_fields), subplot_kw={'projection': ccrs.PlateCarree()})
    cmap = plt.get_cmap('turbo')
    plt.set_cmap(cmap)
    ...
    for i, field in enumerate(fields_to_plot):
        ...
        if field == 'velocity':
            ...
            cbar1 = fig.colorbar(cset1, ax=ax1, orientation='horizontal')
            ...
            cbar2 = fig.colorbar(cset2, ax=ax2, orientation='horizontal')
