...
# the functions in here are used to make the final forcing files and run the hindcast 
...
# now we are back to the previous code...
    ...
    # now, when using the different servers, make sure to use SLURM or MPIEXEC:
    # so make .in and .sb/.sh for roms, and run roms...

    t1=datetime.now()
    os.chdir('../sdpm_py_util')
    print('does it know server?')
    print(MI['server']) 
    server = 'rockfish' 
    
    if server == 'swell': 
        print('driver_run_forecast_LV1: making .in and .sb...')
        runfuns.make_LV1_dotin_and_SLURM( pkl_fnm , 'hind' )
        running_method = 'slurm'
    ...
    print('...done')
    # run command will be
    print(f'now running roms LV1 with {running_method}.')
...
def run_hind_LV2(t1str,pkl_fnm):
    ...
    if server == 'rockfish': 
        print('making .in and .sh...')
        runfuns.make_LV2_dotin_and_exec( pkl_fnm )
        running_method = 'bash'

    print('...done')
    # run command will be
    print(f'now running roms LV2 with {running_method}.')
...
def run_hind_LV3(t1str,pkl_fnm):
    ...
    # now make .in and .sb/.sh for roms, and run roms...
    ...
    if server == 'rockfish': 
        print('making .in and .sh...')
        runfuns.make_LV3_dotin_and_exec( pkl_fnm )
        running_method = 'bash'

        
    print('...done')
    # run command will be
    print(f'now running roms LV3 with {running_method}.')
