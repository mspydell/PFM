...
def driver_run_hind_LV123( input_py_full, pkl_fnm ):
    ...
    print(f'we initialized model from {os.getcwd()}')
    ...
    print('where are we in driver_run_hindLV123 at start?') 
    print(os.getcwd())
    ...
    for time1 in MI['start_times_str']:
        ...
        for lvl in MI['levels_to_run']:
            ...
            print('where are we in driver_run_hindLV123 to process cmd_list?') 
            print(os.getcwd())

            print('changing to driver dir to run hind_simulation...') 
            os.chdir('../driver')
...
def driver_run_hind_LV123_edited_by_Matt( pkl_fnm ):
    t00 = datetime.now()
    MI = initfuns.get_model_info( pkl_fnm )
    nt  = len(MI['start_times_str'])
    nlv = len(MI['levels_to_run'])
